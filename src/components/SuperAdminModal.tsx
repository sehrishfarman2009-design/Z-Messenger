...
export const SuperAdminModal: React.FC<SuperAdminModalProps> = ({
  ...
  const handleBotRequestAction = async (requestId: string, action: "approve" | "reject") => {
    try {
      ...
      const targetUserId = data.request?.userId;
      if (targetUserId) {
        setUsersList((prev) =>
          prev.map((u) =>
            u.id === targetUserId
  ...
  // Directly Toggle a User's Chatbot Access on the Users Table
  const handleToggleUserBotAccess = async (targetUser: AdminUser, allow: boolean) => {
    try {
      ...
      setNotification({
        type: "success",
        text: allow
          ? `Chatbot access granted to @${targetUser.username}.`
                      ...
                      <div className="flex items-center gap-1.5 flex-wrap sm:flex-nowrap justify-end shrink-0 pt-2 sm:pt-0 border-t sm:border-t-0 border-neutral-200">
                        ...
                        {!isSelf && (
                          <button
                            ...
                            title={hasBotAccess ? "Revoke chatbot access" : "Grant chatbot access"}
                    ...
                    <div className="flex items-center justify-end gap-2 pt-1 border-t border-neutral-200/60">
                      {req.status === "pending" ? (
                        <>
                          <button
                            type="button"
                            onClick={() => handleBotRequestAction(req.id, "reject")}
                          ...
                          <button
                            type="button"
                            onClick={() => handleBotRequestAction(req.id, "approve")}
                            disabled={actionInProgressId === req.id}
